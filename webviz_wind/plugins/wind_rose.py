...
class WindRose(WebvizPluginABC):
    ...
    """
    def __init__(self, app, webviz_settings, 
        weibull_input_file: Path = None,
        timeseries_input_file: Path  = None,
        year: int = None,
        nof_years: int = None) -> None:
        ...
        if timeseries_input_file is not None:
            ...
            self.timeseries_df, self.available_years = self.format_timeseries(self.timeseries_df) #make more flexible for more formats
    ...
    def format_timeseries(self, df):                      #NOTE Date, year, Time etc should be defined in a separate class
                            ...
                            wcc.Selectors(
                                ...
                                children=[
                                    wcc.Dropdown(
                                        ...
                                        options=[{'label': i, 'value': i} for i in self.available_years],
                               ...
                               label=f"Time series: Wind direction resolution (degrees)",
                                children=[
                                    wcc.Dropdown(
                                        ...
                                        options=[{"label": i, "value": i} for i in [30, 15, 10,5]],
                                        value=30,
                                   )
                                ]
                            ),
    ...
    def set_callbacks(self, app) -> None:
        @app.callback(
            Output(component_id=self.uuid("ts_wind_rose_figure"), component_property="figure"),
            [
                Input(component_id=self.uuid("wd_resolution"), component_property="value"),
                Input(component_id=self.uuid("radio_button_value"), component_property="value"),
        ...
        def _update_timeseries_wind_rose(delta_wd, intervals, start_year, nof_years):
            ...
            df = extract_years(self.timeseries_df, start_year, nof_years,'year')  #year should be defined in a separate class
            df = wind_rose_from_timeseries(df, delta_wd, intervals)  

            fig = px.bar_polar(df, r="freq", theta="wd",
                   color="ws", template="plotly_dark",
        ...
        @app.callback(
            Output(component_id=self.uuid("wind_speed_figure"), component_property="figure"),
            [
                Input(component_id=self.uuid("start_year"), component_property="value"),
                Input(component_id=self.uuid("nof_years"), component_property="value"),
            ]
        ...
        @app.callback(
            Output(component_id=self.uuid("weibull_wind_rose_figure"), component_property="figure"),
            [
                Input(component_id=self.uuid("radio_button_value"), component_property="value"),
            ]
        )
        ...
        def _update_nof_years_to_choose(start_year: int):
            return [{'label': i, 'value': i} for i in range(1, self.last_year - start_year + 1)]
        ...
        def _update_years_to_choose(nof_years: int):
            return [{'label': i, 'value': i} for i in range(self.first_year, self.last_year - nof_years + 1)]
...
def extract_years(
    ...
    """
    try:
        years = list(range(start_year, start_year + nof_years))     #Years we are interested in
        print(f'Extracting the years: {years}')
        df = df.loc[df[date_column].isin(years)]                    #Extract only the years of interest
        #print(df)
        df['timestep'] = [x*1.0/24.0 for x in range(1, len(df)+1)]   #Number of days 
        #print(df)
    except:
        raise Exception(f"Problems reading from year {start_year} to {start_year + nof_years}. Check your data")
   ...
   30  15-20  0.002067
    ...
    for wd in df['wd'].values:                        #Loop over each of the wind directions
        ...
        for i in range(1, len(intervals)):            #Loop each wind speed interval
            name = f'{intervals[i-1]}-{intervals[i]}' #Name of that interval for plotting    
            wfrec = weibull_cdf(intervals[i],a, k) -weibull_cdf(intervals[i-1],a, k)
...
@CACHE.memoize(timeout=CACHE.TIMEOUT)
def wind_rose_from_timeseries(df: pd.DataFrame, wd_delta: int, intervals: list) -> pd.DataFrame :
    ...
    """
    if 360%wd_delta != 0:
        print(f'ERROR: Delta wind direction must devide 360. You are trying to use {wd_delta}')
        raise Exception
    ...
    wd_width = wd_delta/2
    wd[wd>360-wd_width] -= 360 #Turn angles in the last interval before 360 to negative-angles to make calculations later easier.
...
#            wdict[i][f'{intervals[j-1]}-{intervals[j]}'] = 0
    ...
    for wdi in wds: #For each wind direction and each wind speed interval: Calculate the number of observations
        ...
        try:
            ...
            ws_sub = ws[selection] #Subset of wind speeds with the wanted wind directions
        except:
            ws_sub = []
        ...
        for i in range(1, len(intervals)):
            ...
            try:
                selection = np.where((ws_sub> intervals[i-1]) & (ws_sub <= intervals[i])) #Get the index subset
    ...
    if tot0 != tot:
        print(f'WARNING: The total number of observations {tot0} is not the same as in the rose diagram {tot}')
